...
namespace AtmDonemProjesi
{
    public partial class ParaYatir : Form
    {
        ...
        private void btnParaYatir_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ParaYatirma(musteri.Balance, Convert.ToInt32(txtBakiye.Text));
            }
            catch (Exception ex)
            {
        ...
        {
            btnParaYatir.Enabled = BtnParaCekOpen();
        }
        ...
        private void ParaYatir_FormClosing(object sender, FormClosingEventArgs e)
        {
            this.Hide();
            Home home = new Home();
            home.Show();
        ...
        void ParaYatirma(int bakiye, int tutar)
        {
            if (tutar % 10 != 0)
                MessageBox.Show("Sadece 10 ve katları tutarlarda yatırma yapabilirsiniz.", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            else
            {
                musteri.Balance = musteri.Balance + tutar;
                dbContext.SaveChanges();
                MessageBox.Show($"İşleminiz tamamlanmıştır. Güncel bakiye: {musteri.Balance}", "İşlem Başarılı", MessageBoxButtons.OK, MessageBoxIcon.None);
            }
        }
